...
func createAxes() -> SCNNode {
    ...
    // Make axes
    let Xgeo = SCNGeometry.lineBetweenPoints(pointA: SCNVector3Make(100, 0, 0), pointB: SCNVector3Make(-100, 0, 0) )
    Xgeo.firstMaterial = getMaterial(name: "red")
    zeroBox.addChildNode( SCNNode(geometry: Xgeo) )
    
    let Ygeo = SCNGeometry.lineBetweenPoints(pointA: SCNVector3Make(0,100, 0), pointB: SCNVector3Make(0, -100, 0) )
    Ygeo.firstMaterial = getMaterial(name: "yellow")
    zeroBox.addChildNode( SCNNode(geometry: Ygeo) )
    
    let Zgeo = SCNGeometry.lineBetweenPoints(pointA: SCNVector3Make(0, 0, 100), pointB: SCNVector3Make(0,0,-100) )
